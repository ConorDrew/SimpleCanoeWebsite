...
namespace FSM
{
    public class FRMPart : FRMBaseForm, IForm
    {
        ...
        public void LoadMe(object sender, EventArgs e)
        {
            ...
            UseOverride = Conversions.ToBoolean(get_GetParameter(1));
            ((UCPart)LoadedControl).FromOrder = Entity.Sys.Helper.MakeBooleanValid(get_GetParameter(1));
            ((UCPart)LoadedControl).CurrentPart = App.DB.Part.Part_Get(ID);
            LoadForm(sender, e, this);
            ((UCPart)LoadedControl).SetupSuppliersDatagrid();
            ((UCPart)LoadedControl).SetupStockDatagrid();
            ((UCPart)LoadedControl).SetupQuantityDatagrid();
        }
