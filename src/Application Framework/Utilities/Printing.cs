    ...
    namespace Sys
    {
        public class Printing
        {
            ...
            private byte[] AddLgsrSupplementaryInformation(EngineerVisits.EngineerVisit engineerVisit, DataView dvDefects, int siteId, int pageNo)
            {
                ...
                var cbyteArray = File.ReadAllBytes(template);
                ...
                using (cmm)
                {
                    ...
                    using (wordDoc)
                    {
                        ...
                        if (pageNo == 1)
                        {
                            ...
                            var COMO = App.DB.EngineerVisitAdditional.EngineerVisitAdditionalWorkSheet_GetForVisitDVProper(engineerVisit.EngineerVisitID, (int)Enums.AdditionalChecksTypes.COMOAlarm);
                            var SMOKE = App.DB.EngineerVisitAdditional.EngineerVisitAdditionalWorkSheet_GetForVisitDVProper(engineerVisit.EngineerVisitID, (int)Enums.AdditionalChecksTypes.SmokeAlarm);
                            PrintHelper.ReplaceText(wordDoc, "[Como]", COMO.Count.ToString());
                            PrintHelper.ReplaceText(wordDoc, "[Smoke]", SMOKE.Count.ToString());
                            var dvNccGsr = App.DB.EngineerVisitNCCGSR.EngineerVisitNCCGSR_Get_Friendly((int)engineerVisit.EngineerVisitNCCGSR?.EngineerVisitNCCGSRID);
                        ...
                        var para = wordDoc.MainDocumentPart.Document.Body.ChildElements.First<WP.Paragraph>();
