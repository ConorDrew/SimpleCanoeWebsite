...
namespace FSM.Business.Orders
{
    public class OrderControl
    {
        ...
        private Decimal GetCurrentJobCost(int jobId)
        {
            ...
            foreach (DataRow dr in currentCostByJobId.Table.Rows)
            {
                if (!Helper.MakeStringValid(RuntimeHelpers.GetObjectValue(dr["Part"])).Contains("IBC"))
                {
                    num = new Decimal(Convert.ToDouble(num) + Helper.MakeDoubleValid(RuntimeHelpers.GetObjectValue(dr["Cost"])));
                }
            }
