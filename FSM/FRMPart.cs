...
namespace FSM
{
  public class FRMPart : FRMBaseForm, IForm
  {
    ...
    public FRMPart()
    {
      ...
      this.TheLoadedControl = (IUserControl) new UCPart();
    ...
    void IForm.LoadMe(object sender, EventArgs e)
    {
      ...
      this.UseOverride = Conversions.ToBoolean(this.get_GetParameter(1));
      ((UCPart) this.LoadedControl).FromOrder = Helper.MakeBooleanValid(RuntimeHelpers.GetObjectValue(this.get_GetParameter(1)));
      ((UCPart) this.LoadedControl).CurrentPart = App.DB.Part.Part_Get(this.ID);
      this.LoadForm(RuntimeHelpers.GetObjectValue(sender), e, (IForm) this);
      ((UCPart) this.LoadedControl).SetupSuppliersDatagrid();
      ((UCPart) this.LoadedControl).SetupStockDatagrid();
      ((UCPart) this.LoadedControl).SetupQuantityDatagrid();
    }
    ...
    public int ID
    {
      ...
      set
      {
        ...
        if (this.ID == 0)
        {
          ...
          this.Text = "Part : Adding New...";
        }
        else
        {
          this.PageState = Enums.FormState.Update;
          this.Text = "Part : ID " + Conversions.ToString(this.ID);
        }
      }
    }
