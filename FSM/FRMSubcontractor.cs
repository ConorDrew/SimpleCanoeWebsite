...
namespace FSM
{
  public class FRMSubcontractor : FRMBaseForm, IForm
  {
    ...
    {
      this.Load += new EventHandler(this.FRMSubcontractor_Load);
      this._ID = 0;
      this.InitializeComponent();
      this.TheLoadedControl = (IUserControl) new UCSubcontractor();
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (FRMSubcontractor);
    ...
    void IForm.LoadMe(object sender, EventArgs e)
    {
      ...
      ((UCSubcontractor) this.LoadedControl).CurrentSubcontractor = App.DB.SubContractor.Subcontractor_Get(this.ID);
      this.LoadForm(RuntimeHelpers.GetObjectValue(sender), e, (IForm) this);
      ((UCSubcontractor) this.LoadedControl).SetupPostalRegionsDataGrid();
      ((UCSubcontractor) this.LoadedControl).SetupPostalRegionsDataGridUnTicked();
      ((UCSubcontractor) this.LoadedControl).SetupTrainingQualificationsDataGrid();
      ((UCSubcontractor) this.LoadedControl).SetupEngineerEquipmentDataGrid();
      ((UCSubcontractor) this.LoadedControl).SetupDisciplinariesDataGrid();
      ((UCSubcontractor) this.LoadedControl).SetupAbsenceDataGridGrid();
    }
    ...
    public int ID
    {
      ...
      set
      {
        ...
        if (this.ID == 0)
        {
          ...
          this.Text = "Subcontractor : Adding New...";
        }
        else
        {
          this.PageState = Enums.FormState.Update;
          this.Text = "Subcontractor : ID " + Conversions.ToString(this.ID);
        }
      }
    }
