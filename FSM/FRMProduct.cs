...
namespace FSM
{
  public class FRMProduct : FRMBaseForm, IForm
  {
    ...
    public FRMProduct()
    {
      this.Load += new EventHandler(this.FRMProduct_Load);
      this._ID = 0;
      this.InitializeComponent();
      this.TheLoadedControl = (IUserControl) new UCProduct();
    ...
    void IForm.LoadMe(object sender, EventArgs e)
    {
      ...
      ((UCProduct) this.LoadedControl).CurrentProduct = App.DB.Product.Product_Get(this.ID);
      ((UCProduct) this.LoadedControl).FromOrder = Helper.MakeBooleanValid(RuntimeHelpers.GetObjectValue(this.get_GetParameter(1)));
      this.LoadForm(RuntimeHelpers.GetObjectValue(sender), e, (IForm) this);
      ((UCProduct) this.LoadedControl).SetupSuppliersDatagrid();
      ((UCProduct) this.LoadedControl).SetupAssociateDatagrid();
    }
    ...
    public int ID
    {
      ...
      set
      {
        ...
        if (this.ID == 0)
        {
          ...
          this.Text = "Product : Adding New...";
        }
        else
        {
          this.PageState = Enums.FormState.Update;
          this.Text = "Product : ID " + Conversions.ToString(this.ID);
        }
      }
    }
