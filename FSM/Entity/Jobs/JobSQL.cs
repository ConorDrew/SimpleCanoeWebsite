    ...
    public class JobSQL
    {
        ...
        public Job Insert(Job oJob, SqlTransaction trans)
        {
          ...
          new SqlParameter("@Headline", (object) oJob.Headline)
            ...
            foreach (JobAsset current in oJob.Assets)
            {
                current.SetJobID = (object)oJob.JobID;
                this._database.JobAsset.Insert(current, trans);
            }
            foreach (JobOfWork current in oJob.JobOfWorks)
            {
                current.SetJobID = (object)oJob.JobID;
        ...
        public Job Update(
          ...
          SqlTransaction trans)
        {
            foreach (int enumerator in JobOfWorksRemoved)
            {
                this._database.JobOfWorks.Delete(Conversions.ToInteger(enumerator), trans);
            }

            foreach (int enumerator in EngineerVisitsRemoved)
            {
                this._database.EngineerVisits.Delete(Conversions.ToInteger(enumerator), trans);
            }

            foreach (int enumerator in EngineerVisitsOrdersRemoved)
            {
                this._database.Order.Delete(Conversions.ToInteger(enumerator), trans);
            }
          ...
          new SqlParameter("@Headline", (object) oJob.Headline)
            ...
            foreach (JobOfWork current1 in oJob.JobOfWorks)
            {
                ...
                else
                {
                    ...
                    string JobItemIDs = "";
                    foreach (JobItem current2 in current1.JobItems)
                    {
        ...
        public Job Update(
          ...
          ArrayList EngineerVisitsOrdersRemoved)
        {
            foreach (int enumerator1 in JobOfWorksRemoved)
            {
                this._database.JobOfWorks.Delete(Conversions.ToInteger(enumerator1));
            }

            foreach (int enumerator1 in EngineerVisitsRemoved)
            {
                this._database.EngineerVisits.Delete(Conversions.ToInteger(enumerator1));
            }

            foreach (int enumerator1 in EngineerVisitsOrdersRemoved)
            {
                this._database.Order.Delete(Conversions.ToInteger(enumerator1));
            }
        ...
        private void JobQualificationsLevels_Insert(Job oJob)
        {
            ...
            foreach (DataRow current in oJob.JobQualificationsLevels.Table.Rows)
            {
        ...
        private void JobSheets_Insert(Job oJob, SqlTransaction trans)
        {
            ...
            foreach (DataRow current in oJob.JobSheets.Table.Rows)
            {
                if (Microsoft.VisualBasic.CompilerServices.Operators.ConditionalCompareObjectEqual(current["Tick"], (object)true, false))
                {
                    SqlCommand sqlCommand2 = new SqlCommand();
        ...
        public Job GenerateServiceLetterJob(DataRow r, int customerID, string AMPM, int fuelId = 0)
        {
            ...
            bool boolean = r.Table.Columns.Contains("PatchCheck") && Helper.MakeBooleanValid(r["PatchCheck"]) == true;
