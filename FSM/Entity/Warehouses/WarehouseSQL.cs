...
namespace FSM.Entity.Warehouses
{
    public class WarehouseSQL
    {
        ...
        public void Delete(int WarehouseID)
        {
            ...
            {
                if (Sys.Helper.MakeStringValid(row["Registration"].ToString()).Split('*')[1].Trim() == WarehouseName)
                {
                    App.DB.Van.Delete(int.Parse(row["VanID"].ToString()), true);
                }
            }
        }
        ...
        public Warehouse Insert(Warehouse oWarehouse, DataView LocationsDataView)
        {
            ...
            foreach (DataRow row in App.DB.Van.Van_GetAll(true).Table.Rows)
            {
                ...
                App.DB.Van.Insert(oVan, "", null/* TODO Change to default(_) if this is not a reference type */, true);
            }
            ...
            foreach (DataRow row in App.DB.Van.Van_GetAll(true).Table.Rows)
            {
                ...
                {
                    if (row["Registration"] == locationRow["Registration"])
                    {
                        int locID = Conversions.ToInteger(App.DB.Van.Van_GetAll_For_Warehouse(oWarehouse.WarehouseID).Table.Select("Registration = '" + row["Registration"] + "'")[0]["LocationID"]);
