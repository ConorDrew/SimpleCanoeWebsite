...
namespace FSM.Entity.ContractAlternativeSiteJobOfWorks
{
  public class ContractAlternativeSiteJobOfWorkSQL
  {
    ...
    public ArrayList Get_For_ContractSite_As_Objects(int ContractSiteID)
    {
      ...
      try
      {
        enumerator = this.JobOfWork_Get_For_Contract(ContractSiteID).Table.Rows.GetEnumerator();
        while (enumerator.MoveNext())
        {
          ...
          arrayList.Add((object) new ContractAlternativeSiteJobOfWork()
          {
            ...
            Exists = true,
            SetContractSiteID = RuntimeHelpers.GetObjectValue(current[nameof (ContractSiteID)]),
            SetContractSiteJobOfWorkID = RuntimeHelpers.GetObjectValue(current["ContractSiteJobOfWorkID"]),
    ...
    public void SaveRates(DataView oRates, int ContractSiteJobOfWorkID)
    {
      ...
      try
      {
        enumerator = oRates.Table.Rows.GetEnumerator();
        while (enumerator.MoveNext())
        {
          DataRow current = (DataRow) enumerator.Current;
          this._database.ClearParameter();
