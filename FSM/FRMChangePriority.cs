...
{
  [DesignerGenerated]
  public class FRMChangePriority : Form
  {
    ...
    {
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (FRMChangePriority));
      ...
      this.GroupBox1.Text = "Select Priority";
      ...
      this.Name = nameof (FRMChangePriority);
    ...
    private void btnOK_Click(object sender, EventArgs e)
    {
      ...
      {
        int num = (int) App.ShowMessage("Please select a priority", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
      }
      else if (this.Modal)
        this.Close();
      else
    ...
    private void FRMChangePriority_Load(object sender, EventArgs e)
    {
      ...
      DataRow[] dataRowArray1 = table.Select("Name<>'RC - Recall'");
      ...
      while (index < dataRowArray2.Length)
      {
        ...
        row["ManagerID"] = RuntimeHelpers.GetObjectValue(dataRow["ManagerID"]);
        row["Name"] = RuntimeHelpers.GetObjectValue(dataRow["Name"]);
        DT.Rows.Add(row);
        checked { ++index; }
      }
