...
namespace FSM
{
  public class FRMCustomer : FRMBaseForm, IForm
  {
    ...
    public FRMCustomer()
    {
      this.Load += new EventHandler(this.FRMCustomer_Load);
      this._ID = 0;
      this.InitializeComponent();
      this.TheLoadedControl = (IUserControl) new UCCustomer();
    ...
    internal virtual Panel pnlMain
    {
      get
      {
        return this._pnlMain;
      }
      [MethodImpl(MethodImplOptions.Synchronized)] set
      {
        PaintEventHandler paintEventHandler = new PaintEventHandler(this.pnlMain_Paint);
        Panel pnlMain1 = this._pnlMain;
        if (pnlMain1 != null)
          pnlMain1.Paint -= paintEventHandler;
        this._pnlMain = value;
        Panel pnlMain2 = this._pnlMain;
        if (pnlMain2 == null)
          return;
        pnlMain2.Paint += paintEventHandler;
      }
    }
    ...
    void IForm.LoadMe(object sender, EventArgs e)
    {
      ...
      ((UCCustomer) this.LoadedControl).CurrentCustomer = App.DB.Customer.Customer_Get(this.ID);
      ((UCCustomer) this.LoadedControl).SetupContractsDataGrid();
      ((UCCustomer) this.LoadedControl).SetupPrioritiesDGV();
      ((UCCustomer) this.LoadedControl).SetupRequirements();
      ((UCCustomer) this.LoadedControl).SetupCustomerAuthorityAuditDataGrid();
      ((UCCustomer) this.LoadedControl).SetupCustomerAlertsDataGrid();
    ...
    public int ID
    {
      ...
      set
      {
        ...
        if (this.ID == 0)
        {
          ...
          this.Text = "Customer : Adding New...";
        }
        else
        {
          this.PageState = Enums.FormState.Update;
          this.Text = "Customer : ID " + Conversions.ToString(this.ID);
        }
      }
    }
